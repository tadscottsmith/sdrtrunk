...
package io.github.dsheirer.audio.broadcast.trunktranscribe;
...
/**
 * Audio broadcaster to push completed audio recordings to the Trunk Transcribe call upload API.
 *
 */
public class TrunkTranscribeBroadcaster extends AbstractAudioBroadcaster<TrunkTranscribeConfiguration>
{
    private final static Logger mLog = LoggerFactory.getLogger(TrunkTranscribeBroadcaster.class);
     ...
     */
    public TrunkTranscribeBroadcaster(TrunkTranscribeConfiguration config, InputAudioFormat inputAudioFormat,
    ...
    public void start()
    {
        ...
        /**
         * Trunk Transcribe API does not currently expose a test method.
         */
        if(response != null && response.toLowerCase().startsWith("{"))
        {
            setBroadcastState(BroadcastState.CONNECTED);
        }
        else
        {
            mLog.error("Error connecting to Trunk Transcribe server on startup [" + response + "]");
     ...
     *
     * Trunk Transcribe does not have a test API endpoint, so we look for the incomplete call response.
     */
            ...
            if(isValid(audioRecording) && audioRecording.getRecordingLength() > 0)
            {
                ...
                try 
                {
                    ...
                    if(audioBytes != null)
                    {
                        ...
                        TrunkTranscribeBuilder bodyBuilder = new TrunkTranscribeBuilder();
                            ...
                            .addFile(audioBytes)
                            .addPart(FormField.CALL_JSON, "{'call_length': 2,'call_filename': 'mp3','audio_type': 'digital'}")
                        ...
                        HttpRequest fileRequest = HttpRequest.newBuilder()
                            ...
                            .header(HttpHeaders.AUTHORIZATION, "Bearer testing") //+ //configuration.getApiKey())
                            ...
                            .whenComplete((fileResponse, throwable1) -> {
                                if(throwable1 != null || fileResponse.statusCode() != 200)
                                {
                                    if(throwable1 instanceof IOException || throwable1 instanceof CompletionException)
                                    {
                                        ...
                                        mLog.error("Trunk Transcribe API file upload fail [" +
                                    ...
                                    broadcast(new BroadcastEvent(TrunkTranscribeBroadcaster.this,
                                ...
                                else
                                {
                                    ...
                                    {
                                        //Trunk Transcribe is telling us to skip audio upload - someone already uploaded it
                                        audioRecording.removePendingReplay();
                                    }
                                    else
                                    {
                    ...
                    else
                    {
                        ...
                        mLog.error("Trunk Transcribe API - upload file not found [" +
    ...
    /**
     * Tests both the connection and configuration against the TrunkTranscribe Call API service
     * @param configuration containing API key and system id
     * @return error string or null if test is successful
     */
    public static String testConnection(TrunkTranscribeConfiguration configuration)
    {
    ...
    public static void main(String[] args)
    {
        ...
        TrunkTranscribeConfiguration config = new TrunkTranscribeConfiguration();
        config.setHost("https://api.TrunkTranscribe.com/call-upload-dev");
