...
package io.github.dsheirer.audio.broadcast.trunkingrecorder;
...
/**
 * Audio broadcaster to push completed audio recordings to the TrunkingRecorder call push API.
 *
 * Note: this is not the same as the TrunkingRecorder Feeds (ie streaming) service
 */
public class TrunkingRecorderBroadcaster extends AbstractAudioBroadcaster<TrunkingRecorderConfiguration>
{
    private final static Logger mLog = LoggerFactory.getLogger(TrunkingRecorderBroadcaster.class);
    ...
    private ScheduledFuture<?> mTrunkingRecorderTestFuture;
     ...
     */
    public TrunkingRecorderBroadcaster(TrunkingRecorderConfiguration config, InputAudioFormat inputAudioFormat,
    ...
    public void start()
    {
        ...
        {
            mLog.error("Error connecting to TrunkingRecorder calls server on startup [" + response + "]");
            setBroadcastState(BroadcastState.ERROR);
        }

        if(mTrunkingRecorderTestFuture == null && getBroadcastConfiguration().isTestEnabled())
        {
            // Test periodically so we don't get marked offline due to radio inactivity
            mTrunkingRecorderTestFuture = ThreadPool.SCHEDULED.scheduleAtFixedRate(new TrunkingRecorderTest(), getBroadcastConfiguration().getTestInterval(), getBroadcastConfiguration().getTestInterval(), TimeUnit.MINUTES);
        }
    ...
    public void stop()
    {
        if(mTrunkingRecorderTestFuture != null)
        {
            mTrunkingRecorderTestFuture.cancel(true);
            mTrunkingRecorderTestFuture = null;
        }
        if(mAudioRecordingProcessorFuture != null)
        {
    ...
    public class TrunkingRecorderTest implements Runnable
    {
        @Override
        public void run()
        {
            ...
            if(response != null && response.toLowerCase().startsWith("ok"))
            {
                mLog.info("TrunkingRecorder Calls keep-alive success");
            }
            else
            {
                mLog.info("TrunkingRecorder Calls keep-alive failure");
            }
        }
    }
    ...
    /**
     * Processes any enqueued audio recordings.  The Trunking Recorder calls API uses a two-step process that includes
    ...
    private void processRecordingQueue()
    {
        while(connected() && !mAudioRecordingQueue.isEmpty())
        {
            ...
            if(isValid(audioRecording) && audioRecording.getRecordingLength() > 0)
            {
                ...
                TrunkingRecorderBuilder bodyBuilder = new TrunkingRecorderBuilder();
                    ...
                    mHttpClient.sendAsync(request, HttpResponse.BodyHandlers.ofString())
                        .whenComplete((stringHttpResponse, throwable) -> {
                            if(throwable != null || stringHttpResponse.statusCode() != 200)
                            {
                                ...
                                broadcast(new BroadcastEvent(TrunkingRecorderBroadcaster.this,
                            ...
                            else
                            {
                                ...
                                if(urlResponse.startsWith("0 "))
                                {
                                    ...
                                    {
                                        mLog.error("TrunkingRecorder calls API - audio recording file not found - ignoring upload");
                                    }
                                        ...
                                        mHttpClient.sendAsync(fileRequest, HttpResponse.BodyHandlers.ofString())
                                            .whenComplete((fileResponse, throwable1) -> {
                                                if(throwable1 != null || fileResponse.statusCode() != 200)
                                                {
                                                    ...
                                                    {
                                                        setBroadcastState(BroadcastState.TEMPORARY_BROADCAST_ERROR);
                                                        mLog.error("TrunkingRecorder calls API file upload fail [" +
                                    ...
                                    else
                                    {
                                        ...
                                        mLog.error("TrunkingRecorder calls API - upload file not found [" +
                                ...
                                {
                                    //TrunkingRecorder is telling us to skip audio upload - someone already uploaded it
                                    audioRecording.removePendingReplay();
                                }
                                else
                                {
                                    mLog.error("TrunkingRecorder calls API upload URL request failed [" + urlResponse + "]");
                                    setBroadcastState(BroadcastState.TEMPORARY_BROADCAST_ERROR);
                                    incrementErrorAudioCount();
                                    broadcast(new BroadcastEvent(TrunkingRecorderBroadcaster.this,
                                        BroadcastEvent.Event.BROADCASTER_ERROR_COUNT_CHANGE));
    ...
    /**
     * Tests both the connection and configuration against the TrunkingRecorder Call API service
     * @param configuration containing API key and system id
     * @return error string or null if test is successful
     */
    public static String testConnection(TrunkingRecorderConfiguration configuration)
    {
    ...
    public static void main(String[] args)
    {
        ...
        TrunkingRecorderConfiguration config = new TrunkingRecorderConfiguration();
